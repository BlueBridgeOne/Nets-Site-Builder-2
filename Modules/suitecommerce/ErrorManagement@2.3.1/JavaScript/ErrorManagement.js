...
define(
		'ErrorManagement', [
			...
			, 'ErrorManagement.ExpiredLink.View', 'ErrorManagement.ForbiddenError.View', 'ErrorManagement.InternalError.View', 'ErrorManagement.LoggedOut.View', 'ErrorManagement.PageNotFound.View', 'GlobalViews.Message.View'

			, 'jQuery', 'Backbone', 'underscore', 'Profile.Model'
		...
		function (
			...
			, ExpiredLinkView, ForbiddenErrorView, InternalErrorView, LoggedOutView, PageNotFoundView, GlobalViewsMessageView

			, jQuery, Backbone, _, ProfileModel
		) {
			'use strict';
				...
				mountToApp: function (application) {
					...
					jQuery(document).ajaxError(function (e, jqXhr, options, error_text) {
							...
							if (intStatus === 401 || intStatus === 206) {
								...
								if (jqXhr.responseText.indexOf("SuiteScript") > -1) {
						...
						jQuery(window).on('hashchange', control_valid_navigation); application.once('afterStart', control_valid_navigation);
					}
				};
			});